...
if USE_POSTGRES:
    ...
    class PGCursor:
        ...
        # Tables whose primary key is NOT "id" (so INSERT ... RETURNING id would
        # fail with UndefinedColumn). Add table names here if you add more.
        _NO_ID_TABLES = {"settings"}

        def execute(self, query, params=()):
            pg_query = query.replace("?", "%s")
            stripped = pg_query.strip().upper()
            insert_match = re.match(r"INSERT\s+INTO\s+(\w+)", stripped)
            table_name = insert_match.group(1).lower() if insert_match else None
            wants_returning_id = (
                stripped.startswith("INSERT")
                and "RETURNING" not in stripped
                and table_name not in PGCursor._NO_ID_TABLES
            )
